...
class Cross_Section_Rad:
	...
	# Tp (eV) -> kinetic energy of CR proton.
		...
		sigma_inel=30.7-0.96*np.log(self.Tp/self.pc.Tpth)+0.18*pow(np.log(self.Tp/self.pc.Tpth),2) 
		sigma_inel*=pow(1.0-pow(self.pc.Tpth/self.Tp,1.9),3) 
	...
	# Tp (eV) -> kinetic energy of CR proton.
		...
		sigma_pi[mask1]=self.func_sigma_1pi(self.Tp[mask1])+self.func_sigma_2pi(self.Tp[mask1]) 
		sigma_pi[mask2]=self.func_sigma_inel(self.Tp[mask2])*self.func_npi(self.Tp[mask2]) 
	...
	# Tp (eV) -> kinetic energy of CR proton.
		...
		Amax[mask1]=0.0 
		Amax[mask2]=5.9*self.func_sigma_pi(self.Tp[mask2])/Epi_max[mask2] 
		Amax[mask3]=9.53*pow(theta_p[mask3],-0.52)*np.exp(0.054*pow(np.log(theta_p[mask3]),2))*self.func_sigma_pi(self.Tp[mask3])/self.pc.mp 
		Amax[mask4]=9.13*pow(theta_p[mask4],-0.35)*np.exp(9.7e-3*pow(np.log(theta_p[mask4]),2))*self.func_sigma_pi(self.Tp[mask4])/self.pc.mp 
	...
	# Eg (eV) -> energy of gamma ray.
		...
		F=np.where(mask, pow(1.0-pow(Xg,self.func_alpha()),self.func_beta())/pow(1.0+Xg/C,self.func_gamma()), 0)
	...
	# Tp (eV) -> kinetic energy of CR proton.
		...
		GTp=1.0+np.log(np.maximum(1.0,self.func_sigma_inel()/self.func_sigma_inel(1.0e12*self.Tp**0))) 
	...
	# Tp (eV) -> kinetic energy of CR proton.
		...
		eps_nucl[mask1]=1.7
		eps_nucl[mask2]=1.37+0.39*10.0*np.pi*self.func_GTp(self.Tp[mask2])/self.func_sigma_inel(self.Tp[mask2]) 
	...
	# Eg (eV) -> energy of gamma ray.
		...
		Amax[mask]=self.func_Amax(self.Tp[mask])
